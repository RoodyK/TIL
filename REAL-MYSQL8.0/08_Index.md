...
B-Tree에 저장될 때는 저장될 값을 이용해 B-Tree상 적절한 위치를 검색해야 한다.  
...
그림의 경우 하나의 인덱스 페이지(16KB)에 `16*1024/(16+12) = 585`개의 키를 저장할 수 있다.  
즉, 585개의 자식 노드를 가지는 B-Tree 구조가 된다.  
인덱스의 키가 2배로 커지면 `16*1024/(32+12) = 372`개가 된다.  
...
인덱스를 통한 읽기의 손익분기점을 판단할 필요가 없는데, 보통 DBMS의 옵티마이저에서 인덱스를 통해 레코드를 1건 읽는 것이 테이블에서 직접 레코드를 읽는 것보다 4 ~ 5배 정도 비용이 더 많이 드는 작업인 것으로 예측한다.(DBMS 마다 차이 존재)  
...
위 사례에서 인덱스를 사용하는 것은 손익 분기점보다 훨씬 크기 때문에 이점이 없다. 옵티마이저가 기본적으로 힌트를 무시하고 테이블을 스캔하는 방식으로 처리하겠지만 알아두면 좋다.  
...
MySQL 서버는 멀티 밸류 인덱스의 지원 없이 JSON 타입의 컬럼만 지원했는데, 8.0 버전부터 지원되기 시작했다.  
...
데이터 레코드가 저장된 주소는 내부적 레코드 아이디(ROW ID) 역할을 한다. 그리고 PK나 세컨더리 인덱스의 각 키는 ROW ID를 이용해 실제 데이터 레코드를 찾아온다.  
MyISAM, MEMORY 테이블의 클러스터링 키와 세컨더리 인덱스는 구조적으로 차이가 없다.  

InnoDB 테이블에서 클러스터링 키 값이 변경될 때마다 데이터 레코드의 주소가 변경되고 그때마다 테이블의 모든 인덱스에 저장된 주소값을 변경해야 할 것이다.  
이런 오베허드를 제거하기 위해 InnoDB 테이블(클러스터링 테이블)의 모든 세컨더리 인덱스는 해당 레코드가 저장된 주소가 아니라 프라이머리 키 값을 저장하도록 구현돼 있다.  
...
이렇게 자동으로 추가된 컬럼은 사용자에게 보이지 않기 때문에 접근(사용)힐 수 없다.  
...
MyISAM, MEMORY 테이블에서 PK는 NULL이 허용되지 않은 유니크 인덱스와 같지만, InnoDB 테이블의 PK는 클러스터링 키 역할도 하므로 유니크 인덱스와는 근본적으로 다르다.  
...
유니크 인덱스와 일반 인덱스를 중복해서 생성해 둔 경우가 가끔 있는데, 같은 역할을 하므로 중복해서 인덱스를 생성할 필요는 없다.  
...
여기서 부모 테이블이 트랜잭션을 시작하고 자식 테이블이 참조하는 레코드를 제거하면(fid=1) 부모 테이블은 자식 테이블이 쓰기 잠금이 해제될 때까지 기다려야 한다.  
