 ...
 * 확장 프로퍼티
 *
 * String 클래스에 새로운 속성 lastChar를 추가
 * 코틀린에서는 get()과 set() 메서드를 사용해 속성에 대한 접근 방식을 정의할 수 있다.
 * 자바에서는 보통 필드에 접근하기 위해 메서드를 사용하지만, 코틀린에서는 필드처럼 속성에 접근할 수 있다.
 * get()을 사용해서 값을 구할 방법을 정의한다. 문자열의 마지막 문자를 반환한다.
 */
