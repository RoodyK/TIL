...
물리적 모델링은 논리적 모델을 특정 데이터베이스 관리 시스템(DBMS)의 특성과 성능을 고려하여 구체적인 저장 구조인 물리적 스키마로 변환하는 과정이다.  
논리적 모델링이 데이터 구조의 정합성과 안정성에 초점을 맞췄다면, 물리적 모델링은 실제 시스템의 성능, 저장공간의 효율성, 유지보수 편의성 등을 극대화하는 데 목표를 둔다.  

### 물리적 모델링 프로세스

1. 논리적 모델의 한글 이름을 영문 이름으로 변환한다. 테이블명, 컬럼명을 정하는 규칙을 정하고 적용한다.
2. 각 컬럼에 작헙한 데이터 타입을 선택한다. 이는 저장 공간과 성능에 직접적인 영향을 미친다.
3. 기본 키, 외래 키, NOT NULL 등 데이터의 무결성을 보장하기 위해 제약 조건을 구체적으로 설정한다.
4. 데이터 조회 성능을 극대화하기 위해 어떤 컬럼에 인덱스를 생성할지 결정한다.  
5. 필요에 따라 정규화 원칙을 위배하여 테이블을 통합하거나 중복 데이터룰 추가하는 역정규화를 수행하여 성능을 개선한다.(최후의 수단)
6. 대용량 테이블의 경우에 특정 기준으로 데이터를 분할하여 저장하는 파티셔닝 같은 고급 기법을 고려한다.

<br/>

## 테이블과 컬럼 변환 규칙 - 기본

개발에서 절대적인 규칙은 없다. 프로젝트의 종류나 규모에 따라서 규칙은 달라질 수 있다. 우리 프로젝트에 적용할 규칙에 대해 진지하게 고민하고, 결정했다면 그 것을 일관성 있게 적용한다.  

### 일반적인 명명 규칙

1. 영어를 사용한다.
2. 소문자와 스네이크 케이스(snake_case)를 사용한다.
3. 명확하고 서술적인 이름 사용: 이름만 보고 그 역할과 의미를 명확히 알 수 있도록 짓는다.
4. 일관성 있는 접두사와 접미사: 이름에 일관성을 부여하면 가독성이 크게 향상된다. PK는 테이블명_id(member_id, product_id), 날짜(created_at, modified_at)
5. ORDER, GROUP와 같은 예악어 사용을 금지한다.
...
3. 날짜/시간 컬럼은 접미사로 용도를 명확히 한다.
   - `_at`: 특정 작업이 발생한 정확한 시점(날짜와 시간)을 기록할 떄 사용한다.
   - `_dt`: 일시, 날짜와 시간 정보가 중요할 떄 사용한다.
   - `_date`: 날짜 정보에만 사용한다. 시간 정보가 필요 없거나 의미가 없는 경우 적합하다.
   - `_time`: 시간 정보에만 사용한다. 날짜와 관계없이 특정 시간 정보가 필요할 떄 사용한다.
4. Boolean 타입 컬럼은 'is_' 또는 'has_' 접두사를 사용한다.
5. 컬럼은 단수 명사를 사용한다.
6. 의미 있는 축약어를 사용한다. (quantity -> qty, identifier -> id)
7. 데이터의 성격을 명확히 표현한다. 
    - 쇼핑몰이 여러 국가를 지원한다면 price 뒤에 `_krw`, `_usd` 등
    - 무게를 다룬다면 `weight_kg`, `weight_g`
    - 문맥도 명확히 제공해야 한다. 상품명은 `product` 테이블의 `name` 컬럼을 할 수 도 있지만 `JOIN`을 생각해서 `product_name`으로 하는것이 명확

위에 제시된 규칙들은 널리 사용되는 좋은 관례지만 절대적인 법칙은 아니며, 가장 중요한 것은 일관성이다.

<br/>

## 테이블과 컬럼 변환 규칙 - 축약어와 단수 복수

축약어는 타이핑을 줄여준다는 편리함이 있지만, 불분명한 축약어는 의미 파악이 힘들어 유지보수 비용을 증가시키는 기술 부채로 쌓이게 된다.  

### 좋은 축약어의 조건

- 보편성: id(identifier), avg(average), max(maximum)처럼 누가봐도 그 의미를 알 수 있는 축약어여야 한다.  
- 비모호성: 반드시 하나의 의미로만 해석되야 한다. `auth` 같은 경우 authentication, authorization 두 가지로 혼용될 수 있으니 풀네임을 사용한다.
- 일관성 및 문서화: 특정 비즈니스 도메인에 통용되느 축약어를 사용해야 한다면, 반드시 프로젝트의 데이터 사전이나 용어 사전에 그 의미를 명확히 기록해야 한다.

### 나쁜 축약어가 초래하는 비용

- 유지보수 비용 증가: 축약어의 의미를 파악하기 위해 추가적인 시간이 소모된다.
- 버그 발생 가능성 증가: 축약어를 오해하면 잘못된 쿼리를 작성해 심각한 버그로 이어질 수 있다.

### 단수와 복수

우리나라에서는 단수를 주로 사용하는데, 가장 중요한 것은 회사에서 사용하는 규칙을 정해서 통일하는 것이 좋다.

<br/>

## 데이터 타입 - 문자, 숫자, PK 타입

논리적 모델의 '속성'을 물리적 모델의 '컬럼'으로 바꿀 때, 가장 먼저 해야 할 일은 데이터 타입을 지정하는 것이다.  

### 잘못된 데이터 타입의 문제

- 저장 공간 낭비: 필요보다 더 큰 데이터 타입을 사용하면 디스크 공간이 낭비된다. 데이터가 많아질수록 낭비는 무시할 수 없다.
- 성능 저하: 데이터베이스는 디스크에서 메모리로 데이터를 읽어와 처리한다. 데이터 타입이 크면 한 번에 읽어올 수 있는 데이터의 양이 줄어든다. 이는 더 많은 I/O를 유발하고, 쿼리 속도를 느리게 한다.
- 데이터 무결성 훼손: 데이터의 성격에 맞지 않는 타입을 사용하면 잘못된 데이터가 입력될 수 있다.

### 문자 타입

대부분의 경우 VARCHAR를 사용한다.  
DBMS는 문자열에 대해 메모리에 임시 공간을 할당할 때 VARCHAR에 설정된 최대 길이를 기준으로 삼는 경우가 많으니 `VARCHAR`의 길이를 설정할 때는, 이 데이터가 가질 수 있는 합리적인 최대 길이를 신중히 고민해서 사용해야 한다.


### 숫자 타입(정수) 

- `TINYINT` 는 상태 값(예: 0=대기, 1=처리중, 2=완료)이나 한정된 범위의 숫자에 사용하면 공간을 매우 효율적으
  로 쓸 수 있다.
- 일반적 PK는 INT로 충분한 경우가 많은데, `INT UNSIGNED`는 음수를 제외하고 양수 부분을 두배로 사용할 수 있다.

### 숫자 타입(소수)

- `DECIMAL(M, D)`: 금융 계산처럼 정확한 소수점 계산이 필요할 때 사용한다.
- `DOUBLE`, `FLOAT`: 부동 소수점 타입. 과한 계산이나 근사치가 허용되는 빠른 계산에 사용된다.

### PK 타입 설정

PK의 타입은 `BIGINT`를 권장하는데 현대에는 데이터의 크기가 `INT`를 넘어갈 때도 있고, `BIGINT`는 `INT`에 비해 디스크 저장 공간이 큰 것이다. 현대에는 디스크 성능이 발달해 디스크 공간이 과거처럼 문제가 되지 않는다.  
데이터 베이스에서 성능에 가장 큰 영향을 미치는 것은 I/O(디스크 입출력) 작업이다.  

실무에서는 핵심 테이블의 PK는 `BIGINT`를 권장한다.   

<br/>

### 데이터 타입 - 날짜, 시간

- `DATE`: 날짜만 저장한다 'YYYY-MM-DD'
- `TIME`: 시간만 저장한다. 'HH:MM:SS'
- 'DATETIME': 날짜와 시간을 모두 저장한다. 'YYYY-MM-DD HH:MI:SS'
...
- 특별한 이유가 없다면 `TIMESTAMP` 대신에 `DATETIME` 을 사용하는 것이 더 직관적이고 안전하다.
  - TIMESTAMP는 시간대 변환/범위(1970~2038) 제약이 있어 비즈니스 시각 기록은 `DATETIME` 이 일반
    적으로 안전하다.
- 테이블에서 생성일과 수정일은 기본이다.
  - 생성일과 수정일 자동화는 테이블을 생성할 때 `DEFAULT CURRENT_TIMESTAMP`, `ON UPDATE CURRENT_TIMESTAMP`를 사용할 수 있다.

### 비즈니스 날짜 vs 자동 생성 날짜

주문 테이블에 `ordered_at`과 `created_at`이 있다. 이 둘의 차이는 실무에서 중요하다.

- `ordered_at`: 회원이 '주문'이라는 비즈니스 행위를 한 시점이다.
- `created_at`: 주문 데이터가 우리 데이터베이스 테이블에 '생성'된 시점이다.

대부분의 경우 두 시간은 거의 동일하겠지만, 데이터 마이그레이션이나 지연 처리 등으로 인해 달라질 수 있으므로 목적에 따라 컬럼 이름을 명확히 구분하면 데이터의 의미를 혼동 없이 정확하게 파악할 수 있다.  

<br/>

## 역정규화

역정규화(Denormalization)는 정규화의 원칙을 의도적으로 위배하는 것이다.  

정규화의 가장 큰 단점은 테이블이 잘게 분리된다는 점이다. 예를 들어 주문 테이블에서 상품명을 조회할 때 주문 항목 및 상품 테이블까지 조인해야 하는데, `JOIN` 연산은 데이터베이스에 부하를 주어 시스템 전체 성능을 저하시키는 주범이 될 수 있다.  
이러한 조회 성능 문제를 해결하기 위해 역정규화가 등장했다.

### 역정규화(Denormalization)란?

데이터의 조회 성능을 향상시키기 위해, 의도적으로 데이터 모델의 정규화 원칙을 위반하여 데이터의 중복을 허용하는 프로세스다.  

역정규화는 데이터의 일관성을과 정합성을 일부 희생하고, 조회 속도를 얻는 일종의 트레이드오프 관계에 있다. 그러므로 신중한 접근이 필요하다.  

역정규화는 조회 성능을 위해 쓰기 속도와 데이터 일관성을 일부 희생하는 선택이므로, 정규화 모델링을 기본으로 해서 성능 테스트를 통해 명확한 병목 지점이 확인됐을 때 최후의 수단으로 신중하게 적용해야 한다.  

### 역정규화 시 데이터 일관성 유지

역정규화는 데이터 불일치의 위험이 있으므로, 원본 데이터가 변경됐을 때, 중복된 데이터를 빠짐없이 찾아서 업데이트 해야 한다.  
이는 애플리케이션 로직, 데이터베이스 트리거, 배치 작업 등을 사용해서 반드시 동기화가 필요하다.  

### 역정규화는 언제 사용하는가?

1. **섣부르게 적용하지 마라**: 설계 단계에서부터 역정규화를 고려하는 것은 위험하다. 먼저 정규화 원칙에 따라 데이터 모델을 설계하는 것이 기본이다.
2. **데이터로 증명하라**: '느릴 것 같다'는 추축으로 역정규화를 진행하면 안 된다. 현대의 장비들은 성능이 매우 좋다. 반드시 서비스 오픈 전 성능 테스트 또는 애플리케이션을 운영하면서 실제 성능 측정을 통해 병목이 되는 쿼리를 명확히 식별해야 한다.
3. **읽기/쓰기 비율을 고려하라**: 역정규화는 보통 조회 성능을 위해 쓰기 성능과 데이터 정합성을 희생하는 구조다. 그러므로 쓰기 작업보다 읽기 작업이 압도적으로 많은 경우에 적용하는 것이 효과적이다.
4. **비용을 계산하라**: 역정규화를 통해 얻는 성능적 이점과, 그로 인해 발생하는 데이터 불일치 문제 해결 및 유지보수를 위한 개발 비용을 철저히 비교 분석해야 한다.  

역정규화는 정규화된 모델에서 시작하여, 실제 운영 환경에서 발생한 성능 문제를 해결하기 위한 최후의 수단으로 사용되어야 한다.  

정규화는 논리적 모델링에서 진행하지만, 역정규화는 실제 데이터베이스의 성능, 저장 공간 등을 고려해서 물ㄹ리적은 저장 구조를 설계하는 물리적 모델링 단계에서 적용하는 기법이다.  

<br/>

## 테이블 정의서

테이블 정의서는 데이터베이스의 각 테이블과 컬럼이 무엇을 의미하고, 어떤 규칙을 가지고 있는지 상세하게 설명하는 '설계 명세서'와 같다.  

### 테이블 정의서의 역할

테이블 정의서는 단순한 문서가 아니라, 프로젝트의 성공에 기여하는 중요 자산이다.

1. 개발자, DBA, 기획자, 데이터 분석가 등 프로젝트에 참여하는 모든 사람이 데이터에 대해 동일한 이해를 갖도록 돕는 '공통 언어' 역할을 한다.
2. 시간이 흘러 시스템이 복잡해깆고 담당자가 바뀌어도, 테이블 정의서만 보면 누구나 데이터베이스 구조를 빠르게 파악하고 유지보수 작업을 수행할 수 있다.
3. 데이터 품질 보증: 각 컬럼의 제약 조건, 기본값, 허용되는 값의 범위 등을 명시함으로써 데이터의 일고나성과 품질을 유지하는 데 기여한다.  
4. 테이블 정의서만 보면 누구나 데이터베이스의 전체 구조를 이해하고, 테이블을 생헐할 수 있어야 한다.
5. 개발자는 이 문서를 보고 테이블과 컬럼의 이름을 정확히 ㅎ사용해서 SQL을 작성하고, 애플리케이션 코드를 구현한다.


### 테이블 정의서의 구성 요소(회사마다 다름)
...
테이블 정의서는 데이터베이스 스키마가 변경될 때마다 반드시 함께 업데이트되어야 하는 '살아있는 문서'다.